...
# Intercepts session exit ONLY when ralph1 loop is active in THIS session
...
# No state file → not a ralph1 session
if [[ ! -f "$STATE_FILE" ]]; then
  exit 0
fi

# Check if THIS session is actually a ralph1 session
# by looking for the ralph1 marker in the transcript
...
# Look for ralph1 marker in transcript (set by setup script output)
if ! grep -q 'RALPH1-SESSION-ACTIVE' "$TRANSCRIPT_PATH" 2>/dev/null; then
  # This session was NOT started by ralph1 — allow exit
  exit 0
fi
...
if [[ -n "$LAST_LINE" ]]; then
  ...
  if echo "$LAST_OUTPUT" | grep -qiE '(natural boundary|no.*(more|further).*(strateg|data|approach)|exhausted.*search.*space|<ralph1-complete>)' 2>/dev/null; then
    echo "Ralph1: Loop completed — Claude determined no further strategies available." >&2
    ...
    echo "Final results: $STRATEGIES_FOUND validated / $STRATEGIES_REJECTED rejected / $TOTAL total" >&2
